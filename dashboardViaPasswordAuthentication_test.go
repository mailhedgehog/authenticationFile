...
func TestDashboardAuthenticateViaPasswordSetPassword(t *testing.T) {
	config := &contracts.AuthenticationConfig{}
	config.Dashboard.ViaPasswordAuthentication.Enabled = true
	auth := CreateFileAuthentication(&StorageConfiguration{Path: filePath}, config)
	...
	(*gounit.T)(t).AssertTrue(auth.Dashboard().ViaPasswordAuthentication().Authenticate("user1", fakePasswords[0].pass))
	(*gounit.T)(t).AssertFalse(auth.Dashboard().ViaPasswordAuthentication().Authenticate("user1", "foo_bar"))

	(*gounit.T)(t).AssertNotError(auth.Dashboard().ViaPasswordAuthentication().SetPassword("user1", "foo_bar"))

	(*gounit.T)(t).AssertFalse(auth.Dashboard().ViaPasswordAuthentication().Authenticate("user1", fakePasswords[0].pass))
	(*gounit.T)(t).AssertTrue(auth.Dashboard().ViaPasswordAuthentication().Authenticate("user1", "foo_bar"))
}
